...
from frontend.admin_console.views.accounts import REGION_LABELS, SYMBOLS, show_flash
...
from backend.ingestion.jobs import create_job, enqueue_job, get_job, list_jobs
from backend.tenancy.provision import add_user, get_tenant, set_config, set_status
...
def _tab_users(t: dict) -> None:
    try:
        users = auth_client.users_of_tenant(t["id"])
    ...
    if users:
        ...
        if c2.button("Reset", key=f"reset_{t['id']}", use_container_width=True):
            ...
            try:
                auth_client.admin_set_password(user["id"], new)
            except AuthError as e:
                st.error(str(e))
            else:
