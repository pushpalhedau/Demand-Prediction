...
export function Insight({
  ...
  return (
    <article className="grid grid-cols-[2rem_minmax(0,1fr)] gap-x-4 gap-y-3 border-t py-5 first:border-t-0 sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
      <span className="font-heading tabular text-muted-foreground/60 pt-0.5 text-lg leading-none font-semibold">{String(rank).padStart(2, "0")}</span>
      <div className="min-w-0 space-y-2">
        ...
        <h3 className="font-heading text-[15px] leading-snug font-semibold tracking-[-0.005em] text-balance">{title}</h3>
        <p className="text-muted-foreground max-w-3xl text-sm leading-relaxed">{detail}</p>
        <div className="flex flex-wrap items-center gap-x-5 gap-y-1">{meta}</div>
      </div>
      <div className="col-start-2 sm:col-start-3 sm:text-right">
        <p className="font-heading tabular text-success text-xl leading-none font-semibold tracking-[-0.02em]">{value}</p>
        <p className="text-muted-foreground mt-1 text-xs">{valueCaption}</p>
      </div>
    </article>
  );
}
