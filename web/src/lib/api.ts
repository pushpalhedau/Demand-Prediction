...
/**
 * The only module that talks to the backend. Sessions live in httpOnly cookies the browser attaches itself, so no
 * token ever passes through this code. Every request carries the CSRF header the API requires.
 */
...
export async function api<T>(
  path: string,
  options: { method?: "GET" | "POST"; body?: unknown; params?: Params; signal?: AbortSignal } = {},
): Promise<T> {
