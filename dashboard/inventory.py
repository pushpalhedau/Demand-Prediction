...
def render_inventory(filters: dict):
    ...
    try:
        ...
        total_holding = df_inv['estimated_holding_cost_usd'].sum()
        ...
        if not reorder_df.empty:
            ...
            st.dataframe(
                reorder_df[['Brand', 'Model', 'State', 'Current Stock', 'Reorder Point', 'Deficit', 'Urgency Score', 'Days in Stock']]
        ...
        with sub_col1:
            ...
            if not slow_df.empty:
                slow_df['Model Label'] = slow_df['brand'] + " " + slow_df['model'] + " (" + slow_df['state'] + ")"
                fig_slow = px.bar(
                    slow_df,
                    y='Model Label',
                    x='days_in_stock',
