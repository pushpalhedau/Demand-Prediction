...
def get_executive_kpis(session: Session, filters: dict = None) -> dict:
    ...
    # Get worst performing region
    region_query = session.query(Sale.region, func.sum(Sale.units_sold).label("cnt"))
    region_query = _apply_sale_filters(region_query, filters)
    worst_region_res = region_query.group_by(Sale.region).order_by(asc("cnt")).first()
    ...
    # Get inventory stockout count and risk count
...
def get_dealer_performance_leaderboard(session: Session, filters: dict = None) -> pd.DataFrame:
    ...
    # Filter dealer attributes if inside filters, otherwise standard sale filters
    query = _apply_sale_filters(query, filters)
...
def get_inventory_status(session: Session, filters: dict = None) -> pd.DataFrame:
    ...
    # Suggestion: Return as dict for API compatibility, then convert to DF in Streamlit
    df = pd.read_sql(query.statement, session.bind)
    return df

def update_inventory_from_csv(session: Session, df: pd.DataFrame):
    """
    New helper function to support the 'Upload CSV' requirement.
    This would bulk insert/update the inventory table.
    """
    # Implementation logic for bulk upsert
    pass

def get_unique_filter_options(session: Session) -> dict:
    """
    Gets lists of unique regions, categories, fuel types, dealers, and years
    to populate filters in Streamlit sidebar.
    """
...
def _apply_sale_filters(query, filters: dict = None):
    """
    Helper to apply general filters to sales-related queries.
    """
    if not filters:
        return query
