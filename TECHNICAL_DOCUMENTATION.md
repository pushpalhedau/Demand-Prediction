...
**Purpose:** Streamlit dashboard for a **UAE automobile dealer group** (one regional group of 24 rooftops across the seven emirates) — demand forecasting, customer/lead intelligence, inventory planning, and news-sentiment demand advisory. Currency is AED; every vehicle is imported (flat 5% GCC customs duty, flat 5% federal VAT). Data is generated by `preprocessing/generate_uae_data.py` (seeded).
...
preprocessing/
  generate_uae_data.py      Seeded synthetic UAE (7-emirate) dataset generator
  ...
  generate_uae_data.py       Seeded synthetic UAE dataset generator
...
realdata-datasets/          Synthetic UAE (7-emirate) "real" mode source CSVs — generated by preprocessing/generate_uae_data.py
...
**Pipeline:**
1. Pull daily `Sale.units_sold` or `Sale.total_revenue_incl_vat`, filtered by category/region/fuel_type/**brand**, reindexed to fill missing dates with 0.
2. Pull the dealer-lever `ExternalFactor` columns — `petrol_95_price_aed_per_litre`, `auto_loan_apr_pct`, `incentive_pct_of_atp`, `inventory_days_supply`, `ramadan_month` — monthly, resample to daily via `.resample('D').ffill()`, merge onto sales by date.
...
| `sentiment_analysis.py` | `sentiment.signal_processor`, `sentiment.analyzers.grok_analyzer`, `sentiment.group_briefing`, `forecasting.prophet_forecasting` | Dealer-facing (repositioned 2026-08-29). A **demand advisory**, not a geopolitical-risk dashboard — see §9. Headline: signed **"expected demand impact, next ~30 days"** (`net_demand_signal_pct`, sales-mix weighted) + a 3-zone headwind/flat/tailwind gauge, then an always-visible deterministic **Bottom line** conclusion (`_bottom_line()`). Two sub-tabs: **Demand Watch** (driver bar by news theme, mix-weighted by-segment bar, scannable signal list, and the **"Generate read"** briefing) and **Does news improve our forecast?** (§9.4). The "Generate read" button runs `group_briefing.build_briefing_context()` (a ~9s sweep of every module's queries, cached 10 min on the filter set) → `generate_group_briefing()` → an 8-section cross-module operating briefing (Grok if enabled, deterministic template otherwise), rendered via `st.text`. Honest empty state, no `_geo_risk_fallback`. Mock-vs-live scorer status is in the post-refresh pipeline-status line. Shared helpers from `utils/helpers.py`. See `docs/changelog/2026-08-29-sentiment-analysis-dealer-positioning.md` |
