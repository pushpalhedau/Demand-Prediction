...
| Credentials and tokens | theft, guessing, forgery | throttled sign-in; JWT `exp/sub/aud` required; tokens only in server memory; no secrets in git or images |
...
* The frontend cannot import the database, ORM or SQLAlchemy: `tests/unit/test_architecture.py` fails the build.
...
3. Put a TLS-terminating reverse proxy in front of the customer app. Add: HSTS, `X-Content-Type-Options: nosniff`,
   `Referrer-Policy: same-origin`, `Content-Security-Policy: frame-ancestors 'none'` (Streamlit needs inline styles and
   websockets, so keep the rest of the policy permissive enough for them).
